...
'Cascade 约束的提取器'
class CascadeConstrExtractor(ConstrExtractor):
    ...
    def getChoiceConditionImp(self,auxVar,known,allDepends,var):
        ...
        if len(var) > 6 and len(allDepends)==0:
            print 'too much variables in a same choice blocks specify one to be true'
            seq = 0
            for v in var:
                print seq,v
                seq = seq + 1
            ans = int(raw_input('seq: '))
            if ans >=0 and ans < len(var):
                known.append(var[ans])
                for v in var:
                    if v == var[ans]:
                        s += v + '== true;\n'
                    else:
    ...
    def getDefaultCondition(self,depend_str,ns):
        ...
        for d in self.node.default:
            ...
            if allDepends == "":
                ...
                ret += '(!(' + allDepends + ")) || " + self.node.var + '==' + dv  +';\n'
#             if ns != '':
...
#             ret += c1
    ...
    def getConditionImp(self,known,rd_dict,condition_dict=None):
        ...
        for depend in allDepends:
            ...
            if depend not in tmpdepends:
                tmpdepends.append('(!'+self.parseCondition(depend) + ')')
